...
const LATEST_SITE_UPDATE_VERSION = '2025-01-12-v1';
...
window.renderPricingTiers = function(p, isPremium) {
    ...
    if (buyPrice > 0) {
        ...
        html += '<span class="text-amber-400 text-xs font-semibold" title="$25k LUX processing fee">🚗 Price:</span>';
        html += '<div class="text-right"><span class="text-amber-400 font-black text-lg">$' + buyPrice.toLocaleString() + '</span><span class="text-amber-400/60 text-[9px] block">+$25k LUX processing fee</span></div>';
        html += '</div>';
    }
